...
export default function SovereignClient({ user }: { user: SovereignUser }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const spotLight = new THREE.SpotLight(0xffeedd, isMobile ? 100 : 120);
      ...
      const materials = [
        ...
        new THREE.MeshPhysicalMaterial({ map: texFront, ...(bumpFront ? { bumpMap: bumpFront } : {}), ...cardMaterialProps }),
        new THREE.MeshPhysicalMaterial({ map: texBack, ...(bumpBack ? { bumpMap: bumpBack } : {}), ...cardMaterialProps }),
      ];
